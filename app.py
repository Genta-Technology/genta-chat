...
if "messages" not in st.session_state:
    st.session_state["messages"] = [
        {"role": "assistant", "content": "Halo, saya adalah asisten anda, ada yang bisa saya bantu?"}]
...
if prompt := st.chat_input():
    ...
    if advanced:
        ...
        response = API.ChatCompletion(
            chat_history=st.session_state.messages[1:], model_name=model_selected, max_new_tokens=1024)
